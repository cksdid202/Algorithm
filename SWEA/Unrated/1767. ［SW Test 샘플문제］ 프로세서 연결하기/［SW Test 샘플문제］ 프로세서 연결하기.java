...
public class Solution {
    ...
    static int N;
    static int[][] map;
    static ArrayList<int[]> cores; // 가장자리가 아닌 코어들의 위치
    ...
    static void dfs(int idx, int cnt, int wireLen) {
        ...
        if (idx == cores.size()) {
            // 연결된 코어 개수가 더 많으면 무조건 갱신
            if (cnt > maxCore) {
                maxCore = cnt;
                minWireLen = wireLen;
            } 
            // 코어 개수가 같으면, 전선 길이가 더 짧은 쪽으로 갱신
            else if (cnt == maxCore) {
        ...
        for (int d = 0; d < 4; d++) {
            ...
            if (count > 0) {
                ...
                // 다음 코어로 이동 (연결 성공했으니 cnt+1, wireLen+count)
                dfs(idx + 1, cnt + 1, wireLen + count);
                
                // 백트래킹 (전선 해제: map을 다시 0으로)
                fill(r, c, d, count, 0);
            }
        }
